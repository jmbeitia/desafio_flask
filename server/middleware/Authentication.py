    ...
    def get_auth_token(*args, **kwargs):
        try:
            ...
            if auth_token is None:
                resp = Response("You are not logged in.",
                                status=401,
            ...
            token = jwt.decode(auth_token,
                               os.environ['USER_JWT_SECRET_KEY'],
                               algorithms=["HS256"])
            g.token = token
            return func(*args, **kwargs)

        except jwt.ExpiredSignatureError:
            return Response("Session expired. Please log back in.",
                            status=403,
                            mimetype="application/json")

        except jwt.InvalidTokenError:
            resp = Response(
               "Your session token is invalid. Please log back in.",
               status=401,
               mimetype="application/json")
            resp.set_cookie("auth_token",
                            "",
...
def authenticate_admin(func):
    ...
    def get_auth_token(*args, **kwargs):
        try:
            ...
            token = jwt.decode(auth_token,
                               os.environ['ADMIN_JWT_SECRET_KEY'],
                               algorithms=["HS256"])
            g.token = token
            return func(*args, **kwargs)
        except jwt.ExpiredSignatureError:
